...
//const real Delaunay::bigscale = 1000.0;
...
void Delaunay::insertapoint(int r) {
	...
	// r	    Index of the point being inserted
	// i, j, k	Vertices of a triangle
	// l     	Fourth point across a shared edge
	// s	    The new point r (inserted point)						
	// tno	    Triangle index in thelist[] containing pts[r]
	// d0, d1, d2	Triangle indices for new triangles
	// ntask	                    Current size of legalization task stack
	// tasks[], taski[], taskj[]	Stack of edges to be legalized
	// key							Hash key for edge lookup in linehash
...
void Delaunay::erasetriangle(int a, int b, int c, int d0, int d1, int d2) {
	...
	thelist[j].stat = 0; // kill/deacitvate this Tiel
