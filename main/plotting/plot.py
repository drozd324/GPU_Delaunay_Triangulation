...
This plotting script has been build as a somewhat general purpose visualisation
program for anything I need with viusualise with the format I created for the
triangulation output from my serial and cuda implementations.
...
save_iteration_all = False
save_iteration_iter = 0
...
with open("tri.txt", "r") as data:
	...
	if (len(sys.argv) == 3 and sys.argv[1] == "save"):
		...
		print(f"[SAVING WITH FILENAME={savefile_name}]")
	if (len(sys.argv) == 3 and sys.argv[1] == "iter" and sys.argv[2] == "all" ):
		only_save_plot = 1
		save_iteration_all = True
		print(f"[SAVING ALL ITERATOINS]")
	...
	while run:
		...
		if save_iteration_all and only_save_plot:
			print(f"[SAVING PLOT ITERATION {save_iteration_iter}]")
			plt.axis("off")
			plt.title("")
			plt.savefig(f"DT_iter{save_iteration_iter}", dpi=max(300, int(num_pts)))
		elif only_save_plot:
			print("[SAVING PLOT]")
			plt.axis("off")
			plt.title("")
		...
		if (len(sys.argv) == 3 and sys.argv[1] == "iter" and sys.argv[2] == "all"):
			iter_idx = save_iteration_iter; 
			save_iteration_iter += 1;
			if iter_idx == len(iter_line_num):
				break;
