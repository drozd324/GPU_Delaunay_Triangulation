	...
	saveFile("./data/data.txt", std::ios_base::app)
{
	...
	std::cout << "supertri nlpts: " << triList[0].nlpts << "\n";
...
int Delaunay::flip(int a, int edge) {
	...
	if (0 > incircle(triList[b].p[opp_idx],
				     triList[a].p[0],
				     triList[a].p[1],
				     triList[a].p[2])) 
	{
	...
	int nlpts_a = triList[a].nlpts;
	int nlpts_b = triList[b].nlpts;
	int nlpts = nlpts_a + nlpts_b;
	Point* lpts = new Point[nlpts];  
	for (int k=0; k<nlpts_a; ++k) {
		lpts[k] = triList[a].lpts[k];
	}
	for (int k=0; k<nlpts_b; ++k) {
		lpts[nlpts_a + k] = triList[b].lpts[k];
	}

	Point p[4] = {triList[a].p[(i-1 + 3)%3], triList[a].p[i], triList[b].p[opp_idx], triList[a].p[(i+1)%3]};
	...
	Point ap[3] = {p[0], p[1], p[2]};
	int an[3] = {n[0], n[1], b};
	int ao[3] = {o[0], o[1], 1};

	Point bp[3] = {p[2], p[3], p[0]};
	int bn[3] = {n[2], n[3], a};
	int bo[3] = {o[2], o[3], 1};

	triList[a].writeTri(lpts, nlpts, ap, an, ao);
	triList[b].writeTri(lpts, nlpts, bp, bn, bo);
...
int Delaunay::insert() {
	...
	for (int i=0; i<max; ++i) {
		...
		Point center = triList[i].lpts[triList[i].center];
		center.print();
		std::cout << "\n";

		if (triList[i].center == -1) { // if center doesnt exist, continue
			continue;
		}

		int nlpts = triList[i].nlpts;
		std::cout << "nlpts: " << nlpts << "\n";
		Point* lpts = new Point[nlpts];

		for (int k=0; k<nlpts; ++k) {
			lpts[k] = triList[i].lpts[k];
		}

		Point p[3] = {triList[i].p[0],
					triList[i].p[1],
					triList[i].p[2]};
		...
		Point p0[3] = {center, p[0], p[1]};
		int n0[3] = {nTri+1, n[0], nTri};
		int o0[3] = {1, o[0], 2};

		Point p1[3] = {center, p[1], p[2]};
		int n1[3] = {i, n[1], nTri+1};
		int o1[3] = {1, o[1], 2};

		Point p2[3] = {center, p[2], p[0]};
		int n2[3] = {nTri, n[2], i};
		int o2[3] = {1, o[2], 2};

		//std::cout << "wow\n";

		triList[nTri  ].writeTri(lpts, nlpts, p1, n1, o1);
		triList[nTri+1].writeTri(lpts, nlpts, p2, n2, o2);
		triList[i     ].writeTri(lpts, nlpts, p0, n0, o0);
...
void Delaunay::initSuperTri() {
	...
	avg.x[0] = avg.x[0]/npts;
	avg.x[1] = avg.x[1]/npts;
	...
	Point p[3] = {Point(center_x + radius*sqrt(4), center_y - radius  ),
		          Point(center_x                 , center_y + 2*radius),
		          Point(center_x - radius*sqrt(3), center_y - radius  )};
	int n[3] = {-1, -1, -1}; 
	int o[3] = {-1, -1, -1}; 
...
//	if (end == false) {
		...
		for (int i=0; i<nTri; ++i) {
			for (int j=0; j<3; ++j) {
				...
				saveFile << triList[i].p[j].x[0] << " " << triList[i].p[j].x[1] << " "; 
			} 
			for (int j=0; j<3; ++j) {
